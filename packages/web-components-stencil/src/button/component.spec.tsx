...
import { TilburgWebcButton } from './component';

const render = (html: string) => newSpecPage({ components: [TilburgWebcButton], html });

describe('tilburg-webc-button', () => {
  it('renders a native button in light DOM with the default classes', async () => {
    const page = await render('<tilburg-webc-button>OK</tilburg-webc-button>');
  ...
  it('maps appearance and size to classes', async () => {
    const page = await render('<tilburg-webc-button appearance="subtle-button" size="small">OK</tilburg-webc-button>');
    const button = page.root!.querySelector('button')!;
  ...
  it('marks submit buttons', async () => {
    const page = await render('<tilburg-webc-button type="submit">Send</tilburg-webc-button>');
    const button = page.root!.querySelector('button')!;
  ...
  it('disables the inner button', async () => {
    const page = await render('<tilburg-webc-button disabled>OK</tilburg-webc-button>');
    const button = page.root!.querySelector('button')!;
  ...
  it('only sets aria-pressed when pressed is set', async () => {
    const plain = await render('<tilburg-webc-button>OK</tilburg-webc-button>');
    expect(plain.root!.querySelector('button')!.hasAttribute('aria-pressed')).toBe(false);
    const pressed = await render('<tilburg-webc-button pressed="true">OK</tilburg-webc-button>');
    expect(pressed.root!.querySelector('button')!.getAttribute('aria-pressed')).toBe('true');
    const unpressed = await render('<tilburg-webc-button pressed="false">OK</tilburg-webc-button>');
  ...
  it('moves title and aria attributes from the host to the inner button', async () => {
    const page = await render('<tilburg-webc-button title="Sluiten" aria-describedby="hint">×</tilburg-webc-button>');
    const button = page.root!.querySelector('button')!;
  ...
  it('prefers an explicit aria-label over the title', async () => {
    const page = await render('<tilburg-webc-button title="Tip" aria-label="Sluiten">×</tilburg-webc-button>');

...
const intro = `A **skip link** is a hidden anchor placed at the very top of a page so keyboard and screen-reader users can jump straight to the main content, bypassing the header, navigation, language toggles, cookie banners, and anything else that comes before the page's actual content. It's the first focusable element on the page, becomes visible the moment it receives keyboard focus, and disappears again when focus moves away.

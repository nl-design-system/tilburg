...
import { TilburgWbcProgressBar } from '@gemeente-tilburg/web-components-react';
import type { Meta, StoryObj } from '@storybook/react';
import { bugs, descriptionWebComponents } from '../../storybook-shared/src/tilburg-progress-bar.examples';

/* Stencil `<tilburg-wbc-progress-bar>` rendered through its generated React
...
} satisfies Meta<typeof TilburgWbcProgressBar>;
  ...
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <div>
        ...
        <TilburgWbcProgressBar value={0} total={5} heading="Welkom" label="Stap 1 van 5" />
      </div>
      <div>
        <h4>40%</h4>
        <TilburgWbcProgressBar value={2} total={5} heading="Persoonlijke gegevens" label="Stap 2 van 5" />
      </div>
      <div>
        <h4>60% — met back-link (default pijl)</h4>
        <TilburgWbcProgressBar
          value={3}
          total={5}
      ...
      <div>
        <h4>100% — voltooid</h4>
        <TilburgWbcProgressBar value={5} total={5} heading="Klaar" label="Stap 5 van 5" />
      </div>
      <div>
        <h4>Alleen track (geen titel/label)</h4>
        <TilburgWbcProgressBar value={3} total={10} aria-label="Voortgang" />
      </div>
    </div>
  ),
};
